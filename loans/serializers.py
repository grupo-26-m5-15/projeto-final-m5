...
def devolution_date():
    ...
    name_day = calendar.day_name[
        datetime.datetime.strptime(str(end_date), "%Y-%m-%d").weekday()
    ]
...
class LoanSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data: dict) -> Loan:
        ...
        if copy.availability is False:
            raise serializers.ValidationError({"Error": "Currently unavailable"})

        book = Book.objects.get(pk=copy.book_id)
        book_availability = book.quantity

        if book_availability < 1:
            Copy.objects.filter(pk=copy.book_id).update(quantity=F("quantity") - 1)
            raise serializers.ValidationError({"Error": "Copy not currently available"})
